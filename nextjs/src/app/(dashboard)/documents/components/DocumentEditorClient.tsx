...
// Documents — client wrapper: autosave, export, AI panel, title editing.
...
export function DocumentEditorClient({
  ...
  const save = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await fetch(`/documents/api/documents/${documentId}`, {
        method: 'PATCH',
  ...
  function handleDocChange(next: TipTapDoc) {
    setDoc(next)
    setDirty(true)
  }
  ...
  async function handleExport() {
    ...
    setError(null)
    try {
      const res = await fetch('/documents/api/export', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      a.href = url
      a.download = `${titleRef.current || 'document'}.docx`
      a.click()
    ...
    <div className="flex flex-col h-screen">
      <div className="flex items-center gap-3 px-4 py-2 border-b border-gray-200 bg-white">
        ...
        <div className="flex items-center gap-2 text-xs text-gray-400">
          {dirty && !saving ? <span>Unsaved changes</span> : null}
          {saving ? (
            <span className="inline-flex items-center gap-1">
              <Loader2 className="h-3 w-3 animate-spin" /> Saving…
            </span>
          ) : null}
          {savedAt && !dirty && !saving ? <span>Saved {savedAt}</span> : null}
        </div>
        <button
          type="button"
          onClick={() => save()}
